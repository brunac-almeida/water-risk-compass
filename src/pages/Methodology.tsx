  ...
  <div className="bg-cream min-h-screen">
    ...
    <section className="max-w-[1240px] mx-auto px-14 pb-20 pt-4 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          {
            label: "Pillar Scores (0–1)",
            tag: "Inputs",
            tagColor: "text-teal",
            text: "Generated by the Python scoring engine using min-max normalization. Fixed values — not affected by sliders. WR = Water Risk, CL = Climate Load, CB = Carbon Impact, EC = Energy Cost.",
          },
          {
            label: "Dynamic Weights (W1–W4)",
            tag: "User-controlled",
            tagColor: "text-risk-amber",
            text: "Controlled by the user via dashboard sliders. Range 0–5. Only the ratio between weights matters — doubling all four weights produces identical results.",
          },
          {
            label: "Final Output (0–10)",
            tag: "Result",
            tagColor: "text-risk-green",
            text: "Lower score = lower risk = better site for your priorities. The ×10 converts the 0–1 weighted average into a more readable 0–10 scale.",
          },
        ].map((row) => (
          <div key={row.label} className="bg-card rounded-xl border border-border p-5">
            <p className={`font-mono text-[11px] uppercase tracking-wider ${row.tagColor} mb-1`}>{row.tag}</p>
            <p className="font-display text-[15px] font-bold text-foreground mb-2">{row.label}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          "The denominator (÷6.5) is the sum of the four default weights (2.0 + 1.0 + 1.5 + 2.0). This value changes automatically as sliders move — the formula always self-normalizes.",
          "Default weights reflect infrastructure siting priorities: water availability and energy cost carry the most weight because they represent the largest long-term operational risks for data centers.",
          "Setting any weight to 0 removes that pillar from the calculation entirely — useful for single-dimension analysis.",
        ].map((note, i) => (
          <div key={i} className="bg-card rounded-xl border border-border p-5">
            <p className="font-mono text-[11px] text-teal mb-2">Note {String(i + 1).padStart(2, "0")}</p>
            <p className="text-[13px] text-slate leading-[1.65]">{note}</p>
          </div>
        ))}
      </div>

      {/* Callout */}
      <div className="bg-card rounded-xl border border-border border-l-4 border-l-teal p-7">
